...
GET_GROUP_PITCH_FUNCTION :: lua_common.LuaFunction {
	...
	func_ptr = proc "c" (L: ^lua.State) -> c.int {
		...
		if lua.gettop(L) < 1 {
			lua.pushstring(L, "get_group_pitch expects at least 1 argument (string)")
			lua.error(L)
			return 0
		}
		...
		group_id := string(lua.tostring(L, 1))
...
SET_GROUP_PITCH_FUNCTION :: lua_common.LuaFunction {
	...
	func_ptr = proc "c" (L: ^lua.State) -> c.int {
		...
		if lua.gettop(L) < 2 {
			lua.pushstring(L, "set_group_pitch expects at least 2 argument (string, number)")
			lua.error(L)
			return 0
		}
